<template>
  <NewBsBox class="beco-page-form-modal-wrapper">
    <!-- 面包屑区域 -->
    <Breadcrumb
      v-if="navList.length"
      :nav-list="navList"
    ...
    >
      ...
      >
        <slot
          name="submit-button"
          v-bind="{submitText, model, handleSubmit}"
        >
          <BeerButton
...
<script lang="ts">
  ...
  import {
    NewBsBox, BeerButton, BeerPanel, Breadcrumb, toaster,
  } from '@xhs/yam-beer'
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      // 格式化传入的表单配置
      const standardConfig = computed(() => formConfig.value.map((configItem:any) => {
        const cnf = {
        ...
        cnf.config.children = configItem.config?.children.map((child:any) => {
          const {
      ...
      const handleSubmit = async () => {
        const validate = () => Promise.all(formItems.map((formItem:any) => formItem.validate()))

        isSubmitting.value = true
        try {
          await validate()
          return props.formSchema?.submit?.(toRaw(model.value))
            .catch(e => toaster.warning(e?.message, { id: 'formModal' }))
            .finally(() => {
